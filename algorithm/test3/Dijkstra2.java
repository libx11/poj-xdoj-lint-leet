...
public class Dijkstra2 extends JFrame
{
    ...
    private MultiwayIndexMinPQ<Double> mpq;//多路堆索引优先队列
    private int D;//设置多路堆
    ...
    //构造方法初始化
    public Dijkstra2(EuclideanGraph G, int D)
    {
        this.G = G;
        this.D = D;
        jta = new JTextArea();
    ...
    private void dijkstra(int s, int d)
    {
        ...
        distTo = new double[V];
        pred = new int[V];
        mpq = new MultiwayIndexMinPQ<>(V, D);
        for (int v = 0; v < V; v++)
        {
            distTo[v] = INFINITY;
            pred[v] = -1;
    ...
    private void relax(int v)
    {
        ...
        while (i.hasNext())
        {
            ...
            if (distTo[w] - EPSILON > distTo[v] + G.distance(v, w))
            {
                ...
                mpq.changeKey(w, distTo[w]);//将更新后的数据放入优先队列中
                pred[w] = v;//w点的上一个点为v
            }
